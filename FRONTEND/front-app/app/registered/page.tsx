        ...
        {loading && (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="h-48 animate-pulse rounded-2xl bg-gray-200 dark:bg-violet-900/20" />
            ))}
          </div>
        )}
            ...
            <div className="flex flex-col gap-4">
              {registrations.map((reg) => {
                ...
                return (
                  <article
                    key={ev.id}
                    className="flex gap-4 rounded-2xl border border-violet-100 bg-white p-4
                               shadow-sm dark:border-violet-900/30 dark:bg-[#1a1730]"
                  >
                    {/* Thumbnail */}
                    <div className="h-24 w-24 shrink-0 overflow-hidden rounded-xl bg-gray-100 dark:bg-violet-950/50">
                      {ev.image ? (
                        ...
                        <div className="flex h-full items-center justify-center text-2xl text-gray-300 dark:text-violet-800/50">
                          🎭
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex flex-1 flex-col justify-between min-w-0">
                      <div>
                        <p className="text-xs text-gray-400 dark:text-violet-400/60 mb-0.5">{ev.source} · {ev.category}</p>
                        <h2 className="font-semibold text-gray-900 dark:text-violet-50 line-clamp-1">{ev.title}</h2>
                        {(ev.location.city || ev.location.name) && (
                          <p className="text-sm text-gray-500 dark:text-violet-300/60 truncate">
                    ...
                    <div className="shrink-0 flex items-start">
                      <button
                        ...
                        className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium
                                   ...
                                   dark:border-violet-900/50 dark:text-violet-400/60 dark:hover:border-red-700 dark:hover:text-red-400"
                      >
                        Cancel
                      </button>
                    </div>
